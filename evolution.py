...
class Individual:
  def __init__(self,
    ...
    # _rnn_layers:  [ num_layers, num_layer_cells ]
  ...
  def encode_structure(self):
    ...
    struct_rnn = np.zeros([2], dtype=np.int32)
    # vector for CNN
    for filter_type in self._cnn_layer.values():
      struct_cnn[filter_type[0]-1] = filter_type[1]
    # vector for RNN
    struct_rnn[0] = self._rnn_layers[0]
    struct_rnn[1] = self._rnn_layers[1]
  ...
  def update_graph(self, word_vocab, char_vocab, max_word_length, epoch):
    ...
    with self._graph.as_default():
      ...
      with tf.device('/gpu:%d' % self._gpu_id):
        ...
        with tf.variable_scope("Epoch_%d_Individual_%d" % (epoch, self._id_number), initializer=initializer):
  ...
  def mutation_struct(self, local=True):
    ...
    for filter_type in self._cnn_layer.values():
      if local:
        num_cnn_type_filters = int(round(np.random.normal(filter_type[1], filter_type[1] * FLAGS.local_std_rate) / FLAGS.cnn_mutate_unit)) * FLAGS.cnn_mutate_unit
        ...
        # add / remove filters
        filter_type[1] = int(round(np.random.randint(FLAGS.min_cnn_type_filters, FLAGS.max_cnn_type_filters+1) / FLAGS.cnn_mutate_unit)) * FLAGS.cnn_mutate_unit
    # mutate filter types
    if local:
      num_cnn_filter_types = int(round(np.random.normal(len(self._cnn_layer), len(self._cnn_layer) * FLAGS.local_std_rate)))
    ...
    if local:
      num_rnn_layer_units = int(round(np.random.normal(len(self._rnn_layers), len(self._rnn_layers) * FLAGS.local_std_rate) / FLAGS.rnn_mutate_unit)) * FLAGS.rnn_mutate_unit
      ...
      num_rnn_layer_units = int(round(np.random.randint(FLAGS.min_rnn_layer_cells, FLAGS.max_rnn_layer_cells+1) / FLAGS.rnn_mutate_unit)) * FLAGS.rnn_mutate_unit
    self._rnn_layers[1] = num_rnn_layer_units
    # mutate number of rnn layers 
    if local:
      num_rnn_layers = int(round(np.random.normal(len(self._rnn_layers), len(self._rnn_layers) * FLAGS.local_std_rate)))
    ...
    self._rnn_layers[0] = num_rnn_layers
  ...
  def mutation(self, local=True):
    # deprecated
    # self.mutation_param()
    self.mutation_struct(local=local)
